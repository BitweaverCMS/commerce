...
<div class="admin bitcommerce">
	...
	<tr>
		...
		<th class="item">{tr}Created{/tr}</th>
		<th class="item">{tr}Purchased{/tr} (%)</th>
		<th {if !empty($smarty.request.tacked)}style="border:2px solid black" class="highlight"{/if}><a href="{$smarty.server.REQUEST_URI}&amp;tacked={if empty($smarty.request.tacked)}1{else}0{/if}">{booticon iname="fa-paper-clip" iexplain="Show Only Highlight Rows"}</a></th>
	</tr>
	...
	<tr {if isset($smarty.request.compare.$statKey)}class="highlight"{/if}>
		...
		<td class="item {$oddeven}" style="">{if !isset($smarty.request.compare.$statKey)}<a href="{$smarty.server.REQUEST_URI}&amp;compare[]={$statKey|urlencode}">{booticon iname="fa-paper-clip" iexplain="Highlight"}</a>{/if}</td>
	</tr>
