		...
		{if count( $addresses )}
			<ul class="unstyled data">
				...
				<li class="item">
					...
					{/if}
					<div class="floaticon">
						<a href="{$smarty.const.BITCOMMERCE_PKG_URL}?main_page=address_book&edit={$addresses[ix].address_book_id}"><i class="icon-pencil"></i></a> 
						<a href="{$smarty.const.BITCOMMERCE_PKG_URL}?main_page=address_book&delete={$addresses[ix].address_book_id}"><i class="icon-trash"></i></a> 
					</div>
	...
	<div class="span6">
		{form action="`$smarty.const.BITCOMMERCE_PKG_URL`index.php?main_page=address_book"}
			{legend legend=$formTitle}
				{include file="bitpackage:bitcommerce/mod_address_edit.tpl" address=$editAddress}
