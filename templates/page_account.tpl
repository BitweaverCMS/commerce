...
<div>
	<a class="btn btn-default" href="{'address_book'|zen_get_page_url}">{booticon iname="fa-home"} {tr}Address Book{/tr}</a></dt>
{if $smarty.const.SHOW_NEWSLETTER_UNSUBSCRIBE_LINK=='true'}
	<a class="btn btn-default" href="{'account_newsletters'|zen_get_page_url}">{booticon iname="fa-envelope"} {tr}Newsletter Subscriptions{/tr}</a></dt>
{/if}
{if $smarty.const.CUSTOMERS_PRODUCTS_NOTIFICATION_STATUS == '1'}
	<a class="btn btn-default" href="{'account_notifications'|zen_get_page_url}">{booticon iname="fa-circle-exclamation"} {tr}Product Notifications{/tr}</a></dt>
{/if}
</div>
