...
<img src="{$product.products_image_url}" class="img-responsive thumbnail" style="max-height:75px"/>
	...
	<td class="text-right">
		...
		<a href="{$smarty.const.BITCOMMERCE_PKG_URL}admin/products_price_manager.php?products_id={$prodId}&amp;current_category_id={$currentCategoryId}">{booticon iname="icon-money"}</a>
	<a href="{$smarty.const.BITCOMMERCE_PKG_URL}admin/{$product.type_handler}.php'page=' . $_GET['page'] . '&product_type=' . $products->fields['products_type'] . '&cPath=' . $cPath . '&pID=' . $products->fields['products_id']	. '&action=new_product_meta_tags') . '">{if zen_get_metatags_keywords($prodId,$smarty.session.languages_id) || zen_get_metatags_description($prodId,$smarty.session.languages_id)}{booticon iname="icon-info"}{else}{booticon iname="icon-info-sign"}{/if}</a>
