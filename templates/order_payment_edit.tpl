		...
		<div class="form-group">
			...
			{forminput id="charge-amount"}
				...
				<div class="input-group">
					...
					<input class="form-control input-sm text-right" type="text" name="payment_amount" value="{$payment.payment_amount|default:$smarty.request.payment_amount|default:$order->getField('amount_due')|escape:"htmlall"}" {if !$gBitUser->hasPermission('p_commerce_admin')}disabled{/if} />
...
{else}
				{formhelp note="Credit Card Number"}
{/if}
			{/forminput}
		</div>
	</div>
