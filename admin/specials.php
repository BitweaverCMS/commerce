	...
	<td width="100%" valign="top"><table>
		...
		<tr>
			<td><table border="0" width="100%" cellspacing="0" cellpadding="0">
			 <tr><?php echo zen_draw_form_admin('search', FILENAME_SPECIALS, '', 'get'); ?>
...
// create split page control
	...
	while (!$specials->EOF) {
		...
		if (isset($sInfo) && is_object($sInfo) && ($specials->fields['specials_id'] == $sInfo->specials_id)) {
			echo '									<tr id="defaultSelected" class="info" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link_admin(FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&sID=' . $sInfo->specials_id . '&action=edit') . '\'">' . "\n";
		} else {
...
switch ($action) {
	case 'delete':
		...
		$contents = array('form' => zen_draw_form_admin('specials', FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&sID=' . $sInfo->specials_id . '&action=deleteconfirm'));
	...
	default:
		...
		if ($sInfo->products_priced_by_attribute == '1') {
			...
			$contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link_admin(FILENAME_CATEGORIES, '&action=new_product' . '&cPath=' . zen_get_product_path($sInfo->products_id, 'override') . '&products_id=' . $sInfo->products_id . '&product_type=' . zen_get_products_type($sInfo->products_id)) . '">' . zen_image_button('button_edit_product.gif', IMAGE_EDIT_PRODUCT) . '<br />' . TEXT_PRODUCT_EDIT . '</a>');
		}
		break;
}
