...
// +----------------------------------------------------------------------+
//	$Id: coupon_admin.php,v 1.20 2010/04/19 21:13:03 spiderr Exp $
//
...
<?php
	...
	case 'voucheredit':
		...
		if( $coupon = $gBitDb->getRow( "SELECT * FROM " . TABLE_COUPONS . " WHERE `coupon_id` = ?", array( $_GET['cid'] ) ) ) {
	...
	case 'new':
		if ($getAction != 'voucheredit' && $_POST['coupon_uses_user'] == '') {
	...
	$gBitSmarty->assign( 'coupon_startdate', $coupon_startdate );
	$gBitSmarty->assign( 'coupon_finishdate', $coupon_finishdate );
		$gBitSmarty->assign( 'startDateSelect', zen_draw_date_selector('coupon_startdate', mktime(0,0,0, $coupon_startdate[1], $coupon_startdate[2], $coupon_startdate[0] ) ) );
 	$gBitSmarty->assign( 'finishDateSelect', zen_draw_date_selector('coupon_finishdate', mktime(0,0,0, $coupon_finishdate[1], $coupon_finishdate[2], $coupon_finishdate[0] ) ) );
	$gBitSmarty->assign( 'languages', zen_get_languages() );
	$gBitSmarty->display( 'bitpackage:bitcommerce/admin_coupon_edit.tpl', HEADING_TITLE );
		break;
	default:
?>
		<td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
...
<?php
		...
		case 'new':
			...
			$contents[] = array('text' => '<br />' . tra( 'Category Restrictions' ) . tra( '(comma seperate list of numbers)' ) .'<br />' . zen_draw_input_field('voucher_number_of'));
			break;
		default:
