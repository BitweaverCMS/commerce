...
class CommercePaymentManager {
	...
	*/
		...
		if (is_array($this->modules)) {
			if ( !empty( $GLOBALS[$this->selected_module] ) && is_object($GLOBALS[$this->selected_module])) {
				if (method_exists($GLOBALS[$this->selected_module], 'update_status')) {
					$GLOBALS[$this->selected_module]->update_status( $pPaymentParameters );
				}
			}
		}
	}
	...
	function confirmation( $pPaymentParameters = NULL ) {
		if (is_array($this->modules)) {
			if ( !empty( $GLOBALS[$this->selected_module] ) && is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
				return $GLOBALS[$this->selected_module]->confirmation( $pPaymentParameters );
			}
		}
	}
	...
	function process_button( $pPaymentParameters = NULL ) {
		if (is_array($this->modules)) {
			if ( !empty( $GLOBALS[$this->selected_module] ) && is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
				return $GLOBALS[$this->selected_module]->process_button( $pPaymentParameters );
			}
		}
	}
	...
	function after_order_create($zf_order_id) {
		...
		if (is_array($this->modules)) {
			if (!empty($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) && (method_exists($GLOBALS[$this->selected_module], 'after_order_create'))) {
