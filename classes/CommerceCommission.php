...
// +----------------------------------------------------------------------+
//  $Id: CommerceCommission.php,v 1.1 2007/04/12 23:51:53 spiderr Exp $
//
			...
			switch( $pParamHash['payment_store']['payment_method'] ) {
				case 'storecredit':
					if( $this->mDb->getAssoc( "SELECT * FROM " . TABLE_COUPON_GV_CUSTOMER . " WHERE `customer_id`=?", array( $pParamHash['payment_store']['payee_user_id'] ) ) ) {
						...
						$this->mDb->query( "INSERT INTO " . TABLE_COUPON_GV_CUSTOMER . " (`amount`,`customer_id`) VALUES (?,?)", array( $pParamHash['payment_store']['payee_user_id'], $pParamHash['payment_store']['payee_user_id'] ) );
					}
					break;
				default:
					break;
