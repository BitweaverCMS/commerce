...
class CommerceVoucher extends BitBase {
	...
	function verify( &$pParamHash ) {
		...
		if( empty( $pParamHash['coupon_name'] ) ) {
			$this->mFeedback['errors'][] = 'No coupon name entered';
		} else {
			$languages = zen_get_languages();
			for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
				$languageId = $languages[$i]['id'];
				if( empty( $pParamHash['coupon_name'][$languageId] ) ) {
					$this->mFeedback['errors'][] = 'No coupon name entered' . $languages[$i]['name'];
				} else {
	...
	function isRedeemable() {
		if( $this->isValid() ) {
			...
			if ( $this->getField( 'uses_per_user' ) ) {
				$query = "SELECT COUNT( `coupon_id` ) AS `redeem_count`, `orders_status` 
				...
				if ( !($redeemCountCustomer['orders_status'] < 0) && $redeemCountCustomer['redeem_count'] >= $this->getField('uses_per_user') && $this->getField('uses_per_user') > 0) {
