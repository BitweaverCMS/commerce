...
// +----------------------------------------------------------------------+
// $Id: tpl_advanced_search_result_default.php,v 1.1 2005/07/05 05:59:28 bitweaver Exp $
//
?>
...
<?php
  ...
  if (isset($_GET['dfrom']) && zen_not_null($_GET['dfrom']) && ($_GET['dfrom'] != DOB_FORMAT_STRING)) {
    $where_str .= " and p.products_date_added >= '" . zen_date_raw($dfrom_to_check) . "'";
  }

  if (isset($_GET['dto']) && zen_not_null($_GET['dto']) && ($_GET['dto'] != DOB_FORMAT_STRING)) {
    $where_str .= " and p.products_date_added <= '" . zen_date_raw($dto_to_check) . "'";
  }
...
?>
     <?php echo '<a href="' . zen_href_link(FILENAME_ADVANCED_SEARCH, zen_get_all_get_params(array('sort', 'page', 'x', 'y')), 'NONSSL', true, false) . '">' . zen_image_button('button_back.gif', IMAGE_BUTTON_BACK) . '</a>'; ?>
