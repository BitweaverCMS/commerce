...
// +----------------------------------------------------------------------+
// $Id: tpl_login_default.php,v 1.1 2005/07/05 05:59:04 bitweaver Exp $
//
?>
<?php echo zen_draw_form('login', zen_href_link(FILENAME_LOGIN, 'action=process', 'SSL')); ?> 
<h1><?php echo HEADING_TITLE; ?></h1>
<?php if ($messageStack->size('login') > 0) { echo '<p>' . $messageStack->output('login') .'</p>'; } ?> 
<?php if ($_SESSION['cart']->count_contents() > 0) { echo '<p>' . TEXT_VISITORS_CART . '</p>'; } ?>

<h3><?php echo HEADING_RETURNING_CUSTOMER; ?></h3>
<fieldset>
	<legend><?php echo CATEGORY_LOGIN; ?></legend>
	<div class="formrow"><label><?php echo ENTRY_EMAIL_ADDRESS . '</label>' .  zen_draw_input_field('email_address'); ?></div>
	<div class="formrow"><label><?php echo ENTRY_PASSWORD . '</label>' .  zen_draw_password_field('password'); ?></div>
	<div class="formrow"><label>&nbsp;</label><?php echo zen_image_submit('button_login.gif', IMAGE_BUTTON_LOGIN); ?></div>
	<div class="formrow"><label>&nbsp;</label><?php echo '<a href="' . zen_href_link(FILENAME_PASSWORD_FORGOTTEN, '', 'SSL') . '">' . TEXT_PASSWORD_FORGOTTEN . '</a>'; ?></div>
</fieldset>
</form> 
