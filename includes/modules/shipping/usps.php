	...
	function quote( $pShipHash = array() ) {
		...
		for ($i=0; $i<$PackageSize; $i++) {
			...
			if ($this->usps_countries == 'US') {
				...
				// International
				if (!empty( $Package['ExtraServices']['ExtraService'] ) && is_array($Package['ExtraServices']['ExtraService'])) {
		...
		if (in_array('Display weight', explode(', ', MODULE_SHIPPING_USPS_OPTIONS))) {
			switch (SHIPPING_BOX_WEIGHT_DISPLAY) {
				case (0):
					...
					break;
				case (1):
					$boxWeight = ' (' . $shipping_num_boxes . ' ' . TEXT_SHIPPING_BOXES . ')';
					break;
				case (2):
					$boxWeight = ' (' . number_format($usps_shipping_weight * $shipping_num_boxes,2) . TEXT_SHIPPING_WEIGHT . ')';
					break;
				default:
					$boxWeight = ' (' . $shipping_num_boxes . ' ' . TEXT_SHIPPING_BOXES . ')	(' . $this->pounds . ' lbs, ' . $this->ounces . ' oz' . ')';
					break;
			}
		}
...
$extraservices .
		...
		if ($this->transitTimeCalculationMode == 'OLD' && $this->getTransitTime && sizeof($transreq) ) {
			...
			$transitTime = $this->parseDomesticLegacyAPITransitTimeResults($transitResp);
		}
