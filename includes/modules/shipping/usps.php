...
// use USPS translations for US shops
	...
	parent::__construct();
		...
		if ($this->enabled) {
			...
			$check_query = $this->mDb->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_USPS_HANDLING_METHOD'");
			if ($check_query->EOF) {
			...
			$check_query = $this->mDb->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_USPS_DECIMALS'");
			if ($check_query->EOF) {
			...
			$this->mDb->Execute("UPDATE " . TABLE_CONFIGURATION . " SET configuration_title= 'Handling Fee - US', configuration_description = 'National Handling fee for this shipping method.' WHERE configuration_key = 'MODULE_SHIPPING_USPS_HANDLING'");

			// check MODULE_SHIPPING_USPS_HANDLING_INT is in
			$check_query = $this->mDb->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_USPS_HANDLING_INT'");
			if ($check_query->EOF) {
