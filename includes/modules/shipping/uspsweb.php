 ...
 *
 */
class uspsweb extends CommercePluginShippingBase {
	/**
	...
	function __construct() { // for older php < 5.1.0 change to usps() instead of __construct()
		...
		if( $this->isEnabled() ) {
			$this->typeCheckboxesSelected = explode(', ', MODULE_SHIPPING_USPSWEB_TYPES);
			...
			$this->getTransitTime = (in_array('Display transit time', explode(', ', MODULE_SHIPPING_USPSWEB_OPTIONS))) ? TRUE : FALSE;
		}
	}
		...
		if( $quotes = $this->isEligibleShipper( $pShipHash ) ) {
			...
			switch(true) {
				...
				default:
					...
					$this->machinable = MODULE_SHIPPING_USPSWEB_MACHINABLE;
			}
			...
			if( empty( $quotes['error'] ) ) {
				...
				if( $destCountryCode == 'US' ) {
					...
					$dOptions = explode(', ', MODULE_SHIPPING_USPSWEB_DMST_SERVICES); // domestic
				...
				// International destination:
					$iExtras = array();
					$iOptions = explode(', ', MODULE_SHIPPING_USPSWEB_INTL_SERVICES);
					...
					if( $this->isCommerceConfigActive( 'MODULE_SHIPPING_USPSWEB_REGULATIONS' ) ) {
						$iInfo = '<div id="iInfo">' . "\n" .
										 '  <div id="showInfo" class="ui-state-error" style="cursor:pointer; text-align:center;" onclick="$(\'#showInfo\').hide();$(\'#hideInfo, #Info\').show();">' . MODULE_SHIPPING_USPSWEB_TEXT_INTL_SHOW . '</div>' . "\n" .
										 '  <div id="hideInfo" class="ui-state-error" style="cursor:pointer; text-align:center; display:none;" onclick="$(\'#hideInfo, #Info\').hide();$(\'#showInfo\').show();">' . MODULE_SHIPPING_USPSWEB_TEXT_INTL_HIDE .'</div>' . "\n" .
					...
					if ($destCountryCode == 'US') {
						if( !empty($Package['SpecialServices']['SpecialService'] ) ) {
							...
							foreach ($Package['SpecialServices']['SpecialService'] as $key => $val) {
								...
								if (isset($dExtras[$val['ServiceName']]) && !empty($dExtras[$val['ServiceName']]) && ((MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online' && strtoupper($val['AvailableOnline']) == 'TRUE') || (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Retail' && strtoupper($val['Available']) == 'TRUE'))) {
						...
						$cost = MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online' && !empty( $Package['CommercialRate'] ) ? $Package['CommercialRate'] : $Package['Rate'];
						...
						if( !empty( $Package['ExtraServices']['ExtraService'] ) ) {
							...
							foreach ($Package['ExtraServices']['ExtraService'] as $key => $val) {
								$val['ServiceName'] = $this->clean_usps_marks($val['ServiceName']);
								if (isset($iExtras[$val['ServiceName']]) && !empty($iExtras[$val['ServiceName']]) && ((MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online' && strtoupper($val['OnlineAvailable']) == 'TRUE') || (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Retail' && strtoupper($val['Available']) == 'TRUE'))) {
									$val['ServiceAdmin'] = $this->clean_usps_marks($iExtras[$val['ServiceName']]);
									$Services[] = $val;
								}
							}
						}
						$cost = MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online' && !empty($Package['CommercialPostage']) ? $Package['CommercialPostage'] : $Package['Postage'];
	...
	//  if (preg_match('#First#', $type)) $type .="RM";
					...
					foreach ($Services as $key => $val) {
						...
						if ($sDisplay == 'Y') $hiddenServices[] = array($Services[$key]['ServiceName'] . ' [' . $Services[$key]['ServiceID'] . ']' => (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online' ? $Services[$key]['PriceOnline'] : $Services[$key]['Price']));
					}
	...
	// Domestic Insurance 100, 101, 125 International 1
					...
					if ($pShipHash['destination']['countries_iso_code_2'] == 'US') {
						...
						// international
						$usps_handling_fee = MODULE_SHIPPING_USPSWEB_HANDLING_INT;
					}
					...
					$cost += (MODULE_SHIPPING_USPSWEB_HANDLING_METHOD == 'Box') ? $usps_handling_fee * $pShipHash['shipping_num_boxes'] : $usps_handling_fee;
					...
					if (in_array('Display transit time', explode(', ', MODULE_SHIPPING_USPSWEB_OPTIONS))) {
					...
					if( ($pShipHash['shipping_weight_box'] <= $maxweight) && ($pShipHash['shipping_weight_box'] > $minweight) ) {
						...
						if ($destCountryCode == 'US' && MODULE_SHIPPING_USPSWEB_FIRST_CLASS_FILTER_US == 'True' && preg_match('#First\-Class#i', $type) && $cnt_first > 1) {
							continue;
						}
	...
	function _getQuote( $pShipHash ) {
		...
		if( $destCountryCode == 'US' ) {
			...
			$request =  '<RateV4Request USERID="' . MODULE_SHIPPING_USPSWEB_USERID . '">' . '<Revision>2</Revision>';
			$package_count = 0;
...
// First-Class Retail and Commercial
			...
			// disable request for First-Class MailRM Letter at > .21875 and not Retail
						if (($requested_type == 'First-Class Mail Letter') && (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Retail') && ($pShipHash['shipping_weight_box'] <= .21875)) {
							$FirstClassMailType = 'LETTER';
			// disable request for First-Class Mail Large Envelope at > 13oz and not Retail  
						} elseif (($requested_type == 'First-Class Mail Large Envelope') && (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Retail') && ($pShipHash['shipping_weight_box'] <= 13/16)) {
						...
						} elseif (($requested_type == 'First-Class Package Service - RetailTM') && (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Retail') && ($pShipHash['shipping_weight_box'] <= 13/16)) {			
			...
			// disable request for First-ClassTM Package Service(existing commercial parcel designation) at > 1 lb and not Online(commercial pricing) 			  
						} elseif (($requested_type == 'First-ClassTM Package Service') && (MODULE_SHIPPING_USPSWEB_RATE_TYPE == 'Online') && ($pShipHash['shipping_weight_box'] <= 15/16)) {
...
//						$Container = 'FLAT RATE BOX';
				...
				$width = BitBase::getParameter( $pShipHash, 'box_width', MODULE_SHIPPING_USPSWEB_WIDTH );
				$length = BitBase::getParameter( $pShipHash, 'box_length', MODULE_SHIPPING_USPSWEB_LENGTH );
				$height = BitBase::getParameter( $pShipHash, 'box_height', MODULE_SHIPPING_USPSWEB_HEIGHT );
				$girth = BitBase::getParameter( $pShipHash, 'box_girth', 2 * (MODULE_SHIPPING_USPSWEB_WIDTH * MODULE_SHIPPING_USPSWEB_WIDTH) );
...
// 
			...
			$width = MODULE_SHIPPING_USPSWEB_WIDTH_INTL;
			$length = MODULE_SHIPPING_USPSWEB_LENGTH_INTL;
			$height = MODULE_SHIPPING_USPSWEB_HEIGHT_INTL;
			$girth = 0;
			...
			$request =  '<IntlRateV2Request USERID="' . MODULE_SHIPPING_USPSWEB_USERID . '">' .
						'<Revision>2</Revision>' .
						'<Package ID="0">' .
						'<Pounds>' . $this->pounds . '</Pounds>' .
						'<Ounces>' . $this->ounces . '</Ounces>' .
...
// defaults
// MODULE_SHIPPING_USPSWEB_LENGTH 8.625
// MODULE_SHIPPING_USPSWEB_WIDTH  5.375
// MODULE_SHIPPING_USPSWEB_HEIGHT 1.625
						'<Width>' . $width . '</Width>' .
						'<Length>' . $length . '</Length>' .
						'<Height>' . $height . '</Height>' .
						'<Girth>' . $girth . '</Girth>' .
...
// Prepare to make quote-request to USPS servers
		...
		switch (MODULE_SHIPPING_USPSWEB_SERVER) {
			case 'production':
		...
		if( preg_match( '#(GXG|International)#i', $service ) ) {
			...
			if( empty( $duration ) ) {
				...
				if( preg_match('#Priority Mail Express#i', $service) ) {
					$duration = '3 - 5 business ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#Priority Mail#i', $service) ) {
					$duration = '6 - 10 business ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#Global Express Guaranteed#i', $service) ) {
					$duration = '1 - 3 business ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#USPS GXG.* Envelopes#i', $service) ) {
					$duration = '1 - 3 business ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#First\-Class#i', $service) ) {
					$duration = 'Varies by destination'; // '' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				}
				/********************* CUSTOM END:  IF YOU HAVE CUSTOM TRANSIT TIMES ENTER THEM HERE ***************/
			} else {
				$duration = preg_replace('/Weeks$/', MODULE_SHIPPING_USPSWEB_TEXT_WEEKS, $duration);
				$duration = preg_replace('/Days$/', MODULE_SHIPPING_USPSWEB_TEXT_DAYS, $duration);
				$duration = preg_replace('/Day$/', MODULE_SHIPPING_USPSWEB_TEXT_DAY, $duration);
			}
			...
			if( empty( $duration ) ) {
				/********************* CUSTOM START:  IF YOU HAVE CUSTOM TRANSIT TIMES ENTER THEM HERE ***************/
				if( preg_match('#Priority Mail Express#i', $service)) {
					$duration = '1 - 2 ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#Priority MailTM#i', $service) ) {
					$duration = '2 - 3 ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#USPS Retail GroundRM#i', $service) ) {
					$duration = '4 - 7 ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#First\-Class#i', $service) ) {
					$duration = '2 - 5 ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				} elseif( preg_match('#Media Mail Parcel#i', $service) ) {
					$duration = '5 - 10 ' . MODULE_SHIPPING_USPSWEB_TEXT_DAYS;
				}
				/********************* CUSTOM END:  IF YOU HAVE CUSTOM TRANSIT TIMES ENTER THEM HERE ***************/
			} else {
...
// Return Receipt Electronic 110 (16) missing
		...
		$serviceOptions = explode(', ', MODULE_SHIPPING_USPSWEB_DMST_SERVICES); // domestic
...
*/
		$iserviceOptions = explode(', ', MODULE_SHIPPING_USPSWEB_INTL_SERVICES);
