<?
/*
  $Id: specialdelivery9am.php,v 1.1 2006/11/01 19:12:17 lsces Exp $
  based upon
  $Id: specialdelivery9am.php,v 1.1 2006/11/01 19:12:17 lsces Exp $
  based upon
  $Id: specialdelivery9am.php,v 1.1 2006/11/01 19:12:17 lsces Exp $
...
// class constructor
    function specialdelivery9am() {
      ...
      $this->icon = (( defined('DIR_WS_ICONS') ? DIR_WS_ICONS : 'images/icons/' ) . 'shipping_sd9am.gif');
      ...
      if ($dest_zone == 0) {
	   ...
	   //12 FEB 04 MBeedell	'glue' together the rates from the 10 cost data entry boxes
        ...
        if ($shipping == -1) {
          ...
          $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_SPECIALDELIVERY9AM_ZONES_HANDLING_' . $dest_zone);
        }
      }
...
//                            'module' => MODULE_SHIPPING_SPECIALDELIVERY9AM_TEXT_TITLE ,
    ...
    global $db;
      if (!isset($this->_check)) {
        $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_SPECIALDELIVERY9AM_STATUS'");
    ...
    global $db;
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_SPECIALDELIVERY9AM_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('', 'MODULE_SHIPPING_SPECIALDELIVERY9AM_ZONES_COST1_1', '0.5:10.40', 'Rates cont\'d (2):', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('', 'MODULE_SHIPPING_SPECIALDELIVERY9AM_ZONES_COST2_1', '1:12.00', 'Rates cont\'d (3):', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('', 'MODULE_SHIPPING_SPECIALDELIVERY9AM_ZONES_COST3_1', '2:14.90', 'Rates cont\'d (4):', '6', '0', now())");
    ...
    global $db;
      ...
      $db->Execute("delete from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_SPECIALDELIVERY9AM_ZONES_COUNTRIES_1'");
    }
