...
// +----------------------------------------------------------------------+
// $Id: products_all_listing.php,v 1.4 2005/08/21 14:34:10 spiderr Exp $
//
  ...
  if ($products_all_split->number_of_rows > 0) {
    ...
    while (!$products_all->EOF) {
      ...
      if (PRODUCT_ALL_LIST_IMAGE != '0') {
        $display_products_image = '<a href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'products_id=' . $products_all->fields['products_id']) . '">' . zen_image( CommerceProduct::getImageUrl($products_all->fields['products_id']), $products_all->fields['products_name'], IMAGE_PRODUCT_ALL_LISTING_WIDTH, IMAGE_PRODUCT_ALL_LISTING_HEIGHT) . '</a>' . str_repeat('<br clear="all" />', substr(PRODUCT_ALL_LIST_IMAGE, 3, 1));
      } else {
