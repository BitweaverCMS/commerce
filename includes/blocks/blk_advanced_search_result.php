...
//  $from_str = "from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m using(manufacturers_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd left join " . TABLE_SPECIALS . " s on p.`products_id` = s.`products_id`, " . TABLE_CATEGORIES . " c, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c";
	...
	if (isset($_REQUEST['categories_id']) && zen_not_null($_REQUEST['categories_id'])) {
		if ($_REQUEST['inc_subcat'] == '1') {
			...
			$where_str .= " and pd.`language_id` = '" . $_SESSION['languages_id'] . "' and p2c.`categories_id` = ? ";
			$bindVars[] = (int)$_REQUEST['categories_id'];
		}
	}

	if (isset($_REQUEST['manufacturers_id']) && zen_not_null($_REQUEST['manufacturers_id'])) {
		$where_str .= " and m.`manufacturers_id` = ? ";
		$bindVars[] = $_REQUEST['manufacturers_id'];
	}
...
// search meta tags
			      $where_str .= " or (mtpd.`metatags_keywords` like '%" . addslashes($search_keywords[$i]) . "%')";
...
//    if ($pto)   $where_str .= " and (IF(s.status = '1', s.specials_new_products_price, p.`products_price`) * if(gz.geo_zone_id is null, 1, 1 + (tr.tax_rate / 100)) <= " . $pto . ")";
		if ($pfrom) {
			$where_str .= " and (p.`lowest_purchase_price` * if(gz.geo_zone_id is null, 1, 1 + (tr.tax_rate / 100)) >= ?)";
			$bindVars[] = $pfrom;
		}
		if ($pto) {
			$where_str .= " and (p.`lowest_purchase_price` * if(gz.geo_zone_id is null, 1, 1 + (tr.tax_rate / 100)) <= ?)";
...
//        $order_str .= "final_price " . ($sort_order == 'd' ? "desc" : "") . ", pd.`products_name`";
	...
	$extra_row = 0;
		...
		$listing = $gBitDb->query( $listing_split->sql_query, $bindVars, MAX_DISPLAY_PRODUCTS_LISTING, $offset );
		while (!$listing->EOF) {
			$rows++;
